...
function StyledImage({ src, alt, width, height }: { src: string, alt: string, width: number, height: number }) {
  return (
    <Image src={src} alt={alt} width={width} height={height} className="mr-2.5 lg:inline-block lg:float-left" />
  )
}

function ButtonLink({ href, target, className, children }: { href: string, target: string, className?: string, children: React.ReactNode }) {
  return (
    <Link 
      href={href} 
      target={target} 
